...
def test_api_address_search_with_empty_results(client):
    ...
    resp = client.get("/api/search/",
                      query_string={"q": "rue Kassim", "citycode": "97610"})  # Mayotte
    assert resp.status_code == 404
    data = resp.json
    assert data == {"message": "IRIS not found from coordinates (45.207312, -12.736401)"}
...
def test_api_city_search_with_empty_results(client):
    # e.g. with DOM-TOM
    resp = client.get("/api/city/search/",
                      query_string={"q": "koungou", "citycode": "97610"})  # Mayotte
    assert resp.status_code == 404
    data = resp.json
    assert data == {"message": 'IRIS not found from coordinates (-12.735749, 45.204971)'}
